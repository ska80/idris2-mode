...
(defcustom idris2-source-locations '()
  "A hack to specify location(s) of idris2 source. This is used
for finding definitions of idris2 symbols that are not in the
current source path for use with idris2-jump-to-def, etc. (It's a
hack because it should be related to the libraries included in
the project being compiled, but it is not. If you switch
projects, you will have to update this)."
  :type '(repeat file)
  :group 'idris2)

(defcustom idris2-warnings-printing (list 'warnings-tree)
  "How to print warnings: tree view ('warnings-tree) in
REPL ('warnings-repl)."
  :group 'idris2
  :type '(repeat symbol)
...
(defface idris2-quasiquotation-face nil
  "The face to be used to highlight quasiquotations in Idris2
source code."
  :group 'idris2-faces)

(defface idris2-antiquotation-face nil
  "The face to be used to highlight antiquotations in Idris2
source code."
  :group 'idris2-faces)
...
(defcustom idris2-mode-hook '(turn-on-idris2-simple-indent
                             turn-on-eldoc-mode)
  "Hook to run upon entering Idris2 mode. You should choose at
most one indentation style."
  :type 'hook
...
(defcustom idris2-mode-lidr-hook '()
  "Hook to run after opening a literate Idris2 file. Use this to
...
(defcustom idris2-hole-list-show-expanded t
  "Show the hole list fully expanded by default. This may be
useful on wide monitors with lots of space for the hole buffer."
  :type 'boolean
  :group 'idris2-hole-list)
...
list are run in order, until one returns non-`nil'.  Set to `nil'
for no banner."
  :type 'hook
  :group 'idris2-repl
...
(defcustom idris2-repl-prompt-style 'short
  "What sort of prompt to show. 'long shows the Idris2 REPL
prompt, while 'short shows a shorter one."
  :options '(short long)
  :type 'symbol
  :group 'idris2-repl)

(defcustom idris2-repl-show-repl-on-startup t
  "If non-`nil', show the REPL window when Idris2 starts. If
`nil', only do this when `idris2-repl' was called interactively."
  :type 'boolean
  :group 'idris2-repl)
