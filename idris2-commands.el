...
(defun idris2-current-buffer-dirty-p ()
  ...
  (or idris2-buffer-dirty-p
      ...
      (and (not idris2-load-to-here)
           (not idris2-loaded-region-overlay))
...
(defun idris2-possibly-make-dirty (beginning end _length)
  ...
  (if (and idris2-load-to-here
           idris2-loaded-region-overlay)
...
(defun idris2-update-loaded-region (fc)
  (if fc
    ...
    ;; HACK: Some versions of Idris2 don't properly return a span for some
    ;; modules, returning () instead. Remove this (and the surrounding (if
    ;; fc)) after Idris2 0.9.17, which contains a fix.
...
(defun idris2-filename-to-load ()
  "Compute the working directory and filename to load in Idris2,
returning these as a cons."
  (let* ((fn (buffer-file-name))
         (ipkg-srcdir (idris2-ipkg-find-src-dir))
         (srcdir (or ipkg-srcdir
...
A prefix argument forces loading but only up to the current
line."
  (interactive "p")
  (save-buffer)
  (idris2-ensure-process-and-repl-buffer)
...
(defun idris2-next-error ()
  ...
  (let ((warnings-forward
         (sort (cl-remove-if-not #'(lambda (w)
                                     (> (overlay-start w) (point)))
                                 idris2-warnings)
               #'(lambda (w1 w2)
                   (<= (overlay-start w1) (overlay-start w2))))))
    (if warnings-forward
...
Idris2 process. This sets the load position to point, if there is
one."
  (save-buffer)
  (idris2-ensure-process-and-repl-buffer)
  (if (buffer-file-name)
...
(defun idris2-thing-at-point (&optional prompt)
  "Return the name, line, and column number at point as a
list. If prompt is t then will prompt user if no symbol is there,
and return what was entered by the user along with the line and
col number. Otherwise, will return nil for whole list.  Use this
in Idris2 source buffers."
  (let ((name
         (or (idris2-thing-at-point-raw)
	...
	(pcase-let* ((`(,name ,file ,line ,col) loc)
	  ...
	  (if (file-exists-p fullpath)
	      (insert-button name 'follow-link t 'button loc
			     'action #'(lambda (_)
                                         (idris2-info-quit)
                                         (idris2-jump-to-location loc is-same-window)))
...
(defun idris2-jump-to-def-same-window ()
  "Same as idris2-jump-to-def but uses same window (note:
previous position can be restored using \\[pop-tag-mark]."
  (interactive)
  (idris2-jump-to-def t))

(defun idris2-type-at-point (thing)
  "Display the type of the name at point, considered as a global
variable."
  (interactive "P")
  (let ((name
         (if thing
...
(defun idris2-print-definition-of-name (thing)
  ...
  (interactive "P")
  (let ((name
         (if thing (read-string "Print definition: ")
           (idris2-name-at-point))))
    (when name
      (idris2-info-for-name :print-definition name))))

(defun idris2-extract-location-from-name (str)
  "Given a string in the form '(name) (possibly 'implementation' at (filename):(startline):(startcol)--(endline):(endcol)',
extracts filename, startline and startcol as a list or nil
otherwise."
  (let* ((index
...
(defun idris2-who-calls-name (name)
  ...
  (let* ((response (car (idris2-eval `(:who-calls ,name))))
         (roots (mapcar #'(lambda (name-children)
                            (apply 'idris2-who-calls-name-helper nil name-children))
                        response)))
    (if (not (null roots))
...
(defun idris2-name-calls-who (name)
  ...
  (let* ((callees (idris2-eval `(:calls-who ,name)))
         (roots (mapcar #'(lambda (c)
                            (idris2-caller-tree c :calls-who))
                        (car callees))))
    (if (not (null roots))
...
(defun idris2-name-calls-who-at-point (thing)
  (interactive "P")
  (let ((name
         (if thing (read-string "Calls who: ")
           (idris2-name-at-point))))
    (when name
...
(defun idris2-browse-namespace (namespace)
  ...
  (interactive
   ;; Compute a default namespace for the prompt based on the text annotations
   ;; at point when called interactively. Overlays are preferred over text
   ;; properties.
   (let ((default
           (or (cl-some #'(lambda (o)
                            (overlay-get o 'idris2-namespace))
                        (overlays-at (point)))
...
(defun idris2-caller-tree (caller cmd)
  "Display a tree from an IDE caller list, lazily retrieving a
few levels at a time."
  (pcase caller
      ...
      :kids #'(lambda ()
                (cl-mapcan #'(lambda (child)
                               (let* ((cmd-to-run (list cmd (car child)))
                                      (child-name (car (idris2-eval `(,cmd-to-run ,(car child))))))
                                 (when child-name
...
(defun idris2-namespace-tree (namespace &optional recursive)
  "Create a tree of the contents of NAMESPACE, lazily retrieving
children when RECURSIVE is non-nil."
  (cl-flet*
    ...
    (let ((highlight `((0 ,(length namespace)
      ...
      (if recursive
        ...
        ;; In the non-recursive case, generate an expanded tree with the first
        ;; level available, but only if the namespace actually makes sense
        (pcase (idris2-eval `(:browse-namespace ,namespace))
          (`((,sub-namespaces ,names . ,_))
           (make-idris2-tree
            :item namespace
...
(defun idris2-docs-at-point (_thing)
  "Display the internal documentation for the name at point,
considered as a global variable."
  (interactive)
  (message "Not implemented yet."))

(defun idris2-eldoc-lookup ()
  "Support for showing type signatures in the modeline when
there's a running Idris2"
  (get-char-property (point) 'idris2-eldoc))

(defun idris2-pretty-print ()
  "Get a term or definition pretty-printed by Idris2.  Useful for
writing papers or slides."
  (interactive)
...
(defun idris2-make-cases-from-hole ()
  ...
  (let ((what (idris2-thing-at-point)))
    (when (car what)
      (save-excursion (idris2-load-file-sync))
      (let ((result
             (car (idris2-eval `(:make-case ,(cadr what) ,(car what))))))
        (if (<= (length result) 2)
...
(defun idris2-case-dwim ()
  "If point is on a hole name, make it into a case
expression. Otherwise, case split as a pattern variable."
  (interactive)
...
(defun idris2-add-clause (proof)
  ...
  (let ((what (idris2-thing-at-point))
    ...
    (when (car what)
      (save-excursion (idris2-load-file-sync))
      (let ((result
             (car (idris2-eval `(,command ,(cadr what) ,(car what)))))
            final-point
        ...
        (while (and (not (eobp))
                    (progn
                      (goto-char (line-beginning-position))
                      ;; this will be true if we're looking at the prefix with
                      ;; extra whitespace
	  ...
	  (save-excursion
	    (let ((start (progn (search-backward "?") (point)))
		  (end
                   (progn
                     (forward-char)
                     (search-forward-regexp "[^a-zA-Z0-9_']") (backward-char) (point))))
  ...
  (let (identifier-start
    ...
    (save-excursion
      (while (and (> (point) (point-min))
                  (idris2-is-ident-char-p (char-before)))
        (backward-char)
...
(defun idris2-kill-buffers ()
  ...
  (let ((bufs
         (list :connection :repl :proof-obligations :proof-shell
               :proof-script :log :info :notes :holes :tree-viewer)))
    (dolist (b bufs)
      (idris2-kill-buffer b))))
...
(defun idris2-quit ()
  "Quit the Idris2 process, cleaning up the state that it has
synchronized with Emacs."
  (interactive)
...
(defun idris2-insert-term-widget (term)
  "Make a widget for interacting with the term represented by
TERM beginning at START-POS in the current buffer."
  (let ((inhibit-read-only t)
    ...
    (let ((term (plist-get (text-properties-at position) 'idris2-tt-term)))
      (if (null term)
        ...
        (let* ((res (car (idris2-eval (list cmd term))))
               ...
               (spans (cadr res))
               (col
                (save-excursion (goto-char (idris2-find-term-end position -1))
                                (current-column)))
               (rendered
                (with-temp-buffer
                  ...
                  (let ((new-tt-term
                         (plist-get (text-properties-at (point-min)) 'idris2-tt-term)))
                    (when new-tt-term
                      (goto-char (point-min))
...
(defun idris2-prove-hole (name &optional elab)
  "Launch the prover on the hole NAME, using Elab mode if ELAB is
non-nil."
  ...
  (run-with-timer 0.25 nil
                  #'(lambda ()
                      (let ((buffer
                             (get-buffer idris2-prover-script-buffer-name)))
                        (when buffer
...
(defun idris2-set-idris2-load-packages ()
  ...
  (let* ((idris2-libdir
         ...
         (idris2-libs
          (cl-remove-if #'(lambda (x) (string= (substring x 0 1) "."))
    ...
    (while (not (string= this-package ""))
      ...
      (setq this-package
            (completing-read prompt
                             (cl-remove-if #'(lambda (x) (member x packages))
                                           idris2-libs))))
    (when (y-or-n-p
           (format "Use the packages %s for this session?"
...
(defun idris2-start-project ()
  ...
  (let* ((project-name (read-string "Project name: "))
    ...
    (when (file-exists-p create-in)
      (error "%s already exists" create-in))
    (when (string= src-dir "")
      (setq src-dir nil))
    ...
    (newline)
    (when src-dir
      (insert "sourcedir = " src-dir) (newline))
    (insert "modules = ")
    (insert first-mod)
    (newline)
    (save-buffer)
...
(defun idris2-set-current-pretty-print-width ()
  "Send the current pretty-printer width to Idris2, if there is a
process."
  (let ((command
         (format ":consolewidth %s"
...
(defun idris2-context-menu-items (plist)
  "Compute a contextual menu based on the Idris2 semantic
decorations in PLIST."
